...
def service_management_create(request):
    ...
    if request.method == 'POST':
        pestcontrolservice = request.POST.get('pestcontrolservice')
        ...
        payment_terms_checkbox = request.POST.get('payment_terms_checkbox')
        ...
        technician_operator_name = request.POST.get('technician_operator_name')
        customer_id = request.POST.get('customer_id')
        customer = customer_details.objects.get(id=customer_id)

        

        # Calculate total charges and payment terms based on GST checkbox
        if gst_checkbox:
            total_charges = float(service_charges) * 1.18  # Adding 18% GST
            payment_terms = "Payment Due in 15 days (including GST)"
        else:
            total_charges = float(service_charges)
            payment_terms = "100% Advance Payment"

        # Create service management object and save to database
        service_management_obj = service_management(
            ...
            payment_terms_checkbox=payment_terms_checkbox,
            lead_date=timezone.now(),
            service_date=service_date,
            sales_person_name=sales_person_name,
            sales_person_contact_no=sales_person_contact_no,
            technician_operator_name=technician_operator_name,
            customer=customer
        )
        service_management_obj.save()
        return redirect('/index')  # Replace with your success URL or redirect
